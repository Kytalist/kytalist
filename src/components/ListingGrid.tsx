...
export function ListingGrid({ items, hrefBase }: Props) {
  const [region, setRegion] = useState<string>("All regions");

  const filtered = useMemo(() => {
    if (region === "All regions") return items;
    ...
    <div className="space-y-8">
      ...
      {filtered.length === 0 ? (
        <p className="text-center text-[#0B4650]/60">
          No listings in this region yet. Try &ldquo;All regions&rdquo; or another
          area.
        </p>
      ) : (
